...
def call_game(calling_number_sequence, N=10):
    ...
    gameState = {'InGame': '0', 'FullHouse': '1', 'FourCorners': '2', 'BottomLine': '3', 'MiddleLine': '4',
                 'TopLine': '5',
                 'EarlyFive': '6'}
...
def run_sumilations(iterations, players=30):
    costs_at_each_turn = [0] *90  # six winning strategies and 90 turns
    for i in range(iterations):
        calling_number_sequence = random.sample(range(1, 91), 90)
        costs_at_each = call_game(calling_number_sequence, players)
        for each in range(2, len(costs_at_each)):
            costs_at_each_turn[each] += costs_at_each[each]

    return costs_at_each_turn
